...
- 初期表示をメインキャラ 1 人に寄せる。
- Q/TAB の住人切替入力を止め、メインキャラ中心の視点にする。
- カメラを島全体の引きよりも、選択キャラ追従のズーム寄りにする。
- 島を広げ、家の配置と道を広い島向けに調整する。
- 島上で家の矩形に入り込まない簡易ブロックを入れる。
- キャラクリックで編集モードに入る導線を追加する。
- 素体に靴パーツを追加し、服編集の中で靴色を変えられるようにする。
